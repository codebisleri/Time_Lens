...
def render_causal_tab():
    st.header("Causal AI — how your business levers *drive* demand")
    st.caption(
        "Standard explainability (SHAP, attention) shows **correlation** — which "
        "feature the model leaned on. Causal AI estimates **causation**: how "
        "much demand would change if you actually moved a lever (price, promo, "
        "scheme days, festival) in the real world — so you can make decisions, "
        "not just read the model.")
    ...
    try:
        ...
        if "Effect Estimation" in task:
                    ...
                    with st.spinner("Estimating effects..."):
                        for t in treatments:
                            try:
                                ...
                                theta = float(e.value)
                                # Decision-grade read-outs: turn the coefficient
                                # into a plain causal sentence + an elasticity
                                # (% demand change per 1% lever change) at the
                                # data means, and a robustness verdict from the
                                # random-common-cause refuter.
                                interp, elas = _causal_interpretation(
                                    t, outcome, theta, features_df)
                                verdict, refute_new = _causal_robustness_verdict(r, theta)
                                estimates.append({
                                    'Treatment (lever)': t,
                                    'Causal Effect (per +1 unit)': theta,
                                ...
                                refutations.append({
                                    'Treatment': t,
                                    'Refutation': str(r),
                                    'Effect after adding random cause': refute_new,
                                })
                                if len(treatments) == 1:
                                ...
                                refutations.append({'Treatment': t,
                                                    'Refutation': f"Error: {ex}",
                                                    'Effect after adding random cause': np.nan})
            ...
            if 'causal_estimate' in st.session_state and st.session_state['causal_estimate'] is not None:
                st.subheader("Causal effects — what each lever *does* to demand")
                st.caption(
                    "Unlike correlation, these estimate the effect of **changing "
                    "the lever in the real world** (holding the selected "
                    "confounders fixed). Use them to decide where to act.")
                st.dataframe(
                    st.session_state['causal_estimate'].style.format({
                        'Causal Effect (per +1 unit)': '{:,.3f}',
                        'Elasticity (% per +1%)': '{:,.2f}',
                    }, na_rep='—'),
                    use_container_width=True, hide_index=True)
                st.caption("**Robustness** re-runs the estimate with an injected "
                           "random confounder — ✓ Robust means the effect barely "
                           "moved; ▲ Caution means it shifted materially, so treat "
                           "the number as directional only.")
                st.subheader("Robustness detail")
