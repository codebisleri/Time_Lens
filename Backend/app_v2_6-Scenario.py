...
SEGMENT_ARCHITECTURE = {
    ...
    'Volatile Low contributors': {
        ...
        'reconcile': 'top_down',
        'cold_start_proxy': True,
        # Holt-Winters is excluded entirely from this pool — it assumes a
        # continuous level + smooth seasonality that intermittent low-volume
        # demand violates, so it tends to over-smooth the zeros. `exclude`
        # is honoured by the candidate-pool builder so HW can't sneak back
        # in via the intermittent safety-net default shortlist.
        'exclude': ['holt_winters'],
        'tagline': 'Croston/SBA (demand occurrence + size) · TSB · DTW proxy fallback',
    },
...
# ════════════════════════════════════════════════════════════════════════
# FORECAST LEVEL — aggregate-then-forecast at any grain (region/category/…)
# ════════════════════════════════════════════════════════════════════════
# The engine is keyed on a single grouping column (`sku_col`). To forecast at
# a higher level than the per-SKU default, we collapse the raw rows to one row
# per (entity, period) where `entity` is built from the planner-chosen group
# columns, then feed that entity key in wherever `sku_col` is used. Profiling,
# panel features, routing, and forecasting are all reused unchanged — they just
# see the aggregated entity as if it were a SKU.
FORECAST_ENTITY_COL = '__forecast_entity__'

# Columns that are properties of the PERIOD (calendar/promo flags), not of the
# item — when collapsing many SKUs into one entity they must NOT be summed
# (they're identical across SKUs for a given date). We keep a representative
# value via max so flags like festive/peak survive the aggregation intact.
_PERIOD_LEVEL_COLS = {
...
def _level_entity_label(cfg) -> str:
    """Human label for the chosen forecast grain (e.g. 'category × region')."""
    mode = cfg.get('forecast_level_mode', 'sku')
    if mode == 'sku':
        return 'SKU'
    if mode == 'overall':
        return 'Overall total'
    cols = cfg.get('forecast_level_cols') or []
    return ' × '.join(cols) if cols else 'group'


def resolve_pipeline_cfg(cfg: dict) -> dict:
    """Return a cfg view for the profiling + forecast pipeline.

    Early tabs (Load Data, EDA) keep the ORIGINAL `cfg` — they work on raw
    per-item rows. From profiling onward the pipeline operates on the
    aggregated panel, so at non-SKU grains we re-point `sku_col` at the derived
    entity key and drop brand/segment (per-item hierarchies that don't survive
    aggregation, and with them brand reconciliation) — unless the planner is
    grouping BY that column, in which case it's kept as part of the entity.
    Returns `cfg` unchanged at the per-SKU default.
    """
...
def aggregate_to_forecast_level(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    """Collapse `df` to one row per (entity, period) for non-SKU forecast levels.

    `cfg['forecast_level_mode']` is one of:
      - 'sku'     → returns `df` unchanged (per-item, the historical default)
      - 'overall' → a single entity covering the whole portfolio
      - 'custom'  → one entity per unique combination of
                    `cfg['forecast_level_cols']` (e.g. category, region, …)

    Sales/revenue/quantity columns are SUMMED; price-like columns are MEANED;
    calendar/promo flags (`_PERIOD_LEVEL_COLS`) keep a representative value; the
    chosen group columns are carried through. The result gains a
    `FORECAST_ENTITY_COL` column which the caller threads in as `sku_col`.
    The `date_col` is expected to already be parsed to datetime by the caller.
    """
    mode = cfg.get('forecast_level_mode', 'sku')
    if mode == 'sku':
        return df
    ...
    if mode == 'overall':
        ...
        if not present:
            # Misconfigured — fall back to per-SKU rather than crash.
            return df
        level_cols = present
    ...
    for c in work.columns:
        ...
        cl = str(c).lower()
        if c in level_cols:
            agg_map[c] = 'first'           # constant within the entity
        elif pd.api.types.is_numeric_dtype(work[c]):
            if 'price' in cl or cl in ('promo_intensity', 'discount', 'margin'):
                agg_map[c] = 'mean'        # rate/level — averaging is sensible
            elif cl in _PERIOD_LEVEL_COLS:
                agg_map[c] = 'max'         # period-level flag, identical per date
            else:
                agg_map[c] = 'sum'         # additive volume (sales, revenue, qty)
        else:
            agg_map[c] = 'first'           # categorical → representative value

    agg = (work.groupby(keys, as_index=False, sort=False)
                .agg(agg_map))
    return agg
...
def build_panel_features(df: pd.DataFrame, date_col: str, sales_col: str,
                         ...
                         exog_categorical: List[str] = None,
                         treat_outliers: bool = False,
                         outlier_k_iqr: float = 3.0,
                         outlier_explain_extra: Optional[List[str]] = None
                         ) -> pd.DataFrame:
    ...
    # ── Unified outlier treatment (one policy for EVERY model) ──────────
    # Runs BEFORE lags/rolling/Fourier and before the global model trains, so
    # the cleaned series is what every algorithm sees. Spikes/dips explained by
    # an active promo/festival/holiday/event/price flag are kept intact.
    if treat_outliers:
        _explain = _resolve_outlier_explain_cols(panel.columns,
                                                 extra=outlier_explain_extra)
        panel, _n_out, _n_evt = apply_unified_outlier_treatment(
            panel, sku_col, sales_col, k_iqr=outlier_k_iqr,
            explain_cols=_explain)
        # Stash a small summary so the caller can surface it in the UI.
        build_panel_features.last_outlier_summary = {
            'treated': _n_out, 'event_kept': _n_evt,
            'explain_cols': _explain, 'k_iqr': outlier_k_iqr,
        }
...
@st.cache_resource
def train_global_lightgbm(_panel: pd.DataFrame, sku_col: str, date_col: str,
                          sales_col: str, freq: str,
                          categorical_cols: List[str],
                          holdout_periods: int = 0,
                          cache_key: str = '') -> Optional[GlobalModelPackage]:
    ...
    PERF: `_panel` has a leading underscore so Streamlit's @st.cache_resource
    SKIPS hashing the (large) DataFrame on every call — hashing a full-catalog
    panel each run was a measurable fixed cost. The cheap `cache_key` string
    (a data signature built by the caller, see run_forecasts) is what actually
    keys the cache, so lookups are O(1) instead of O(panel size).
    """
    if lgb is None:
        return None

    train_panel = _panel.copy()
    ...
    # 'sales_raw' is the pre-cleaning target (added by the unified outlier
    # step) — it MUST be dropped or it leaks the target. 'is_outlier' /
    # 'is_event_outlier' are functions of the target's treatment and are 0 in
    # the future, so they're dropped too rather than filled with 0 at predict.
    drop_cols = {sku_col, date_col, sales_col, 'channel', 'revenue', 'avg_price',
                 'sales_raw', 'is_outlier', 'is_event_outlier'}
...
# Columns whose "active" state EXPLAINS a demand spike/dip — a month flagged by
# any of these is a known driver (promo, festival, holiday, planned event,
# price move), so its value is NOT treated as a statistical outlier. Matched
# case-insensitively; any column starting with 'evt_' (planner event flags) is
# always treated as explanatory in addition to this set.
...
def _resolve_outlier_explain_cols(columns, extra=None) -> List[str]:
    """Pick the explanatory flag columns actually present in `columns`."""
    cols = list(columns)
...
def apply_unified_outlier_treatment(panel: pd.DataFrame, sku_col: str,
    ...
    """One consistent outlier policy applied to the panel BEFORE any model.

    Per entity (`sku_col`), a month is a candidate outlier when its value falls
    outside the robust fence `median ± k_iqr · IQR` (computed within that
    entity, and only for series with ≥ 8 points / non-degenerate IQR). A
    candidate is TREATED — clipped back onto the fence — only when it is NOT
    explained by an active flag in `explain_cols` (promo / festival / holiday /
    planned event / price move). Explained spikes are left untouched so models
    can still learn the genuine event response.

    Because the cleaned values are written back into `sales_col` *before* lags,
    rolling means and the global model train, every algorithm — global LightGBM,
    SARIMAX, Theta, Holt-Winters, Croston, Prophet — trains on the SAME cleaned
    series. The original values are preserved in `sales_raw`; treated rows are
    marked in `is_outlier` and explained-spike rows in `is_event_outlier`.

    Returns (panel, n_treated, n_explained_kept).
    """
    ...
    # Which rows are explained by a known driver → never treated.
...
def forecast_prophet(history: pd.Series, h: int, freq: str,
                     seasonality_mode: Optional[str] = None) -> pd.Series:
    """Meta's Prophet — trend + seasonality + holidays.
    ...
    Seasonality mode (Volatile High contributors): for spiky, high-volume
    demand the seasonal swings SCALE with the level, so **multiplicative**
    seasonality fits far better than additive. When `seasonality_mode` is left
    as None we auto-select multiplicative for volatile (CV>0.5), strictly
    positive series — which is exactly the Volatile High / Volatile Mid
    profile — and additive otherwise. Multiplicative is mathematically invalid
    when the series contains zeros or negatives, so we guard against that and
    ...
    try:
        ...
        _strictly_positive = len(vals) > 0 and float(np.min(vals)) > 0
        if seasonality_mode is None:
            seasonality_mode = ('multiplicative'
                                if (_cv > 0.5 and _strictly_positive)
                                else 'additive')
        elif seasonality_mode == 'multiplicative' and not _strictly_positive:
            # Caller asked for multiplicative but the series has zeros/negatives
            # — Prophet can't fit that; degrade gracefully to additive.
...
#   • theta                → {theta weight, deseasonalize} grid
#   • prophet              → {changepoint_prior_scale, seasonality_mode} grid
#   • croston_sba          → {alpha} smoothing grid (intermittent demand)
#   • tsb                  → {alpha, beta} smoothing grid (intermittent demand)
# Other strategies (chronos, naive, global_lgbm — trained globally across all
# SKUs, so it's tuned at the global-training stage, not here — and autoarima,
# which is already self-tuning) pass through unchanged.
#
...
def fine_tune_winner(
    ...
    # ──────────────────────────────────────────────────────────────
    # THETA grid — theta weight × deseasonalize
    # ──────────────────────────────────────────────────────────────
    if strategy == 'theta':
        try:
            from statsmodels.tsa.forecasting.theta import ThetaModel  # type: ignore
        except Exception:
            return None, None, None, "tuning: ThetaModel unavailable"
        can_seasonal = seasonal_p > 1 and len(train) >= 2 * seasonal_p
        _period = seasonal_p if can_seasonal else None
        grid = [{'theta': th, 'deseasonalize': ds}
                for ds in ([True, False] if can_seasonal else [False])
                for th in (1.5, 2.0, 3.0)]
        best = (None, float('inf'))
        for params in grid:
            try:
                tm = ThetaModel(train, period=_period,
                                deseasonalize=params['deseasonalize']).fit()
                pred = pd.Series(np.clip(tm.forecast(val_h, theta=params['theta']).values, 0, None))
                mape = _safe_mape(val, pred)
                if mape is not None and mape < best[1]:
                    best = (params, mape)
            except Exception:
                continue
        if best[0] is None:
            return None, None, None, "tuning: Theta grid search failed"
        best_params, best_mape = best

        def _theta_apply(series: pd.Series, hh: int, _p=best_params, _sp=seasonal_p):
            try:
                _can = _sp > 1 and len(series) >= 2 * _sp
                tm = ThetaModel(series, period=(_sp if _can else None),
                                deseasonalize=_p['deseasonalize']).fit()
                return pd.Series(np.clip(tm.forecast(hh, theta=_p['theta']).values, 0, None),
                                 index=_future_index(series, hh, freq))
            except Exception:
                return forecast_theta(series, hh, freq)

        future_fc = _theta_apply(history, h)
        return (future_fc, _theta_apply, best_mape,
                f"tuning: Theta grid → theta={best_params['theta']}, "
                f"deseasonalize={best_params['deseasonalize']} (val_WMAPE {best_mape:.1f}%)")

    # ──────────────────────────────────────────────────────────────
    # PROPHET grid — changepoint flexibility × seasonality mode.
    # Kept small: Prophet fits are slow and this runs on the winner only.
    # ──────────────────────────────────────────────────────────────
    if strategy == 'prophet':
        try:
            from prophet import Prophet  # type: ignore
        except Exception:
            return None, None, None, "tuning: prophet not installed"
        grid = [
            {'cps': 0.05, 'mode': 'additive'},
            {'cps': 0.20, 'mode': 'additive'},
            {'cps': 0.50, 'mode': 'additive'},
            {'cps': 0.20, 'mode': 'multiplicative'},
            {'cps': 0.50, 'mode': 'multiplicative'},
        ]
        # Multiplicative seasonality is invalid when the series has zeros or
        # negatives (it scales by the level). Drop those grid points so the
        # search can't crash on — or silently mis-fit — such series. Volatile
        # High contributors are high-volume / strictly positive, so they keep
        # the multiplicative candidates the user wants.
        if float(np.min(np.asarray(train.values, dtype=float))) <= 0:
            grid = [g for g in grid if g['mode'] != 'multiplicative']

        def _prophet_fit_predict(fit_series, steps, cps, mode):
            dd = pd.DataFrame({'ds': fit_series.index, 'y': fit_series.values})
            m = Prophet(yearly_seasonality=True, weekly_seasonality=False,
                        daily_seasonality=False, changepoint_prior_scale=cps,
                        seasonality_mode=mode, seasonality_prior_scale=10.0)
            m.fit(dd)
            fut = m.make_future_dataframe(periods=steps, freq=freq, include_history=False)
            return np.clip(m.predict(fut)['yhat'].values, 0, None)

        best = (None, float('inf'))
        for params in grid:
            try:
                pred = pd.Series(_prophet_fit_predict(train, val_h, params['cps'], params['mode']))
                mape = _safe_mape(val, pred)
                if mape is not None and mape < best[1]:
                    best = (params, mape)
            except Exception:
                continue
        if best[0] is None:
            return None, None, None, "tuning: Prophet grid search failed"
        best_params, best_mape = best

        def _prophet_apply(series: pd.Series, hh: int, _p=best_params):
            try:
                out = _prophet_fit_predict(series, hh, _p['cps'], _p['mode'])
                return pd.Series(out, index=_future_index(series, hh, freq))
            except Exception:
                return forecast_prophet(series, hh, freq)

        future_fc = _prophet_apply(history, h)
        return (future_fc, _prophet_apply, best_mape,
                f"tuning: Prophet grid → changepoint_prior_scale={best_params['cps']}, "
                f"seasonality_mode={best_params['mode']} (val_WMAPE {best_mape:.1f}%)")

    # ──────────────────────────────────────────────────────────────
    # CROSTON / SBA grid — smoothing alpha (intermittent demand)
    # ──────────────────────────────────────────────────────────────
    if strategy == 'croston_sba':
        best = (None, float('inf'))
        for a in (0.05, 0.10, 0.15, 0.20, 0.30):
            try:
                pred = pd.Series(np.clip(sba(train.values, alpha=a, h=val_h), 0, None))
                mape = _safe_mape(val, pred)
                if mape is not None and mape < best[1]:
                    best = (a, mape)
            except Exception:
                continue
        if best[0] is None:
            return None, None, None, "tuning: Croston/SBA grid search failed"
        best_a, best_mape = best

        def _sba_apply(series: pd.Series, hh: int, _a=best_a):
            try:
                arr = sba(series.values, alpha=_a, h=hh)
            except Exception:
                arr = sba(series.values, alpha=0.1, h=hh)
            return pd.Series(np.clip(arr, 0, None), index=_future_index(series, hh, freq))

        future_fc = _sba_apply(history, h)
        return (future_fc, _sba_apply, best_mape,
                f"tuning: Croston/SBA grid → alpha={best_a} (val_WMAPE {best_mape:.1f}%)")

    # ──────────────────────────────────────────────────────────────
    # TSB grid — demand-rate alpha × probability beta (intermittent demand)
    # ──────────────────────────────────────────────────────────────
    if strategy == 'tsb':
        best = (None, float('inf'))
        for a in (0.05, 0.10, 0.20):
            for b in (0.05, 0.10, 0.20):
                try:
                    pred = pd.Series(np.clip(tsb(train.values, alpha=a, beta=b, h=val_h), 0, None))
                    mape = _safe_mape(val, pred)
                    if mape is not None and mape < best[1]:
                        best = ((a, b), mape)
                except Exception:
                    continue
        if best[0] is None:
            return None, None, None, "tuning: TSB grid search failed"
        (best_a, best_b), best_mape = best

        def _tsb_apply(series: pd.Series, hh: int, _a=best_a, _b=best_b):
            try:
                arr = tsb(series.values, alpha=_a, beta=_b, h=hh)
                return pd.Series(np.clip(arr, 0, None), index=_future_index(series, hh, freq))
            except Exception:
                return forecast_tsb(series, hh, freq)

        future_fc = _tsb_apply(history, h)
        return (future_fc, _tsb_apply, best_mape,
                f"tuning: TSB grid → alpha={best_a}, beta={best_b} (val_WMAPE {best_mape:.1f}%)")
...
# =================================================================
# 8b. TOP-DOWN ROUTING — forecast a clean aggregate, split to SKU
#     Used when a SKU is new / cold-start / short-history / lumpy /
#     noisy: the bottom-up model has too little (or too noisy) signal,
#     so we forecast a coarser, cleaner aggregate (e.g. brand or
#     category total) and disaggregate it back to the SKU by its share.
# =================================================================

def _freq_offset(freq: str, n: int):
    """Approximate a DateOffset of `n` periods for the given pandas freq."""
    f = (freq or 'MS').upper()
    if f.startswith('W'):
    ...
    return pd.DateOffset(months=n)  # MS / M default


def robust_series_forecast(hist: pd.Series, horizon: int,
                           freq: str = 'MS') -> Optional[pd.Series]:
    """Robust univariate forecast for an *aggregated* series.

    Holt-Winters when there's enough history (additive trend + seasonal at
    24+ points, trend-only at 12+, simple smoothing at 4+); falls back to a
    seasonal-naive repeat, then a flat tail-mean. Demand is floored at 0.
    Returns a future Series of length `horizon` (date-indexed) or None.
    """
    hist = pd.Series(hist).astype(float).dropna()
    if len(hist) < 2:
        return None
    try:
        future_idx = pd.date_range(hist.index[-1], periods=horizon + 1,
                                   freq=freq)[1:]
    except Exception:
    ...
    try:
        if len(hist) >= 24:
            m = ExponentialSmoothing(hist, trend='add', seasonal='add',
                                     seasonal_periods=12,
                                     initialization_method='estimated').fit()
            vals = np.asarray(m.forecast(horizon), dtype=float)
        elif len(hist) >= 12:
            m = ExponentialSmoothing(hist, trend='add',
                                     initialization_method='estimated').fit()
            vals = np.asarray(m.forecast(horizon), dtype=float)
        elif len(hist) >= 4:
            m = ExponentialSmoothing(hist,
                                     initialization_method='estimated').fit()
            vals = np.asarray(m.forecast(horizon), dtype=float)
    except Exception:
        vals = None
...
def apply_top_down_routing(results: List['ForecastResult'],
                           profiles: pd.DataFrame, df: pd.DataFrame,
                           cfg: Dict[str, Any], horizon: int):
    """Re-forecast hard-to-forecast SKUs top-down, in place.

    For every SKU whose profile matches the planner's selected triggers
    (new/cold-start, short-history, lumpy/intermittent, noisy), the forecast
    is replaced by `group_forecast × sku_share`, where the group is defined by
    `cfg['top_down_levels']` (e.g. brand, or brand × category). Other SKUs keep
    their normal per-SKU forecast.

    Returns (results, summary). `summary['enabled']` is False (no-op) unless
    `cfg['top_down_enabled']` is set and at least one valid level column exists.
    """
    ...
    # --- Which SKUs qualify (read from the profile table) ---
    ...
    # --- sku -> group key (first non-null level values per sku) ---
    ...
    # --- aggregated history + share denominators ---
    df_dt = df.copy()
    df_dt[date_col] = pd.to_datetime(df_dt[date_col], errors='coerce')
    df_dt = df_dt.dropna(subset=[date_col])
    ...
    # Recent window (last 6 periods) for the "recent share" mode.
    try:
    ...
    def _share(sku, gkey):
        ...
        if gt > 0 and st_ > 0:
            return st_ / gt
        # New SKU with no/low history → fall back to an equal split.
        return 1.0 / max(1, int(grp_members.get(gkey, 0)))

    # --- group forecast cache (forecast each group at most once) ---
    ...
    for r in results:
        ...
        new_fc = gfc * sh
        # Keep the original forecast's future index when we have one.
        if r.forecast is not None and len(r.forecast):
            idx = r.forecast.index
            n = min(len(idx), len(new_fc))
            new_fc = pd.Series(new_fc.values[:n], index=idx[:n], name=r.sku)
...
class ForecastResult:
    ...
    # ---- P90 safety-stock guardrail ----------------------------------
    # 90th-percentile (xgb_quantile_90) future forecast, kept whenever that
    # quantile model is in the candidate pool — e.g. the Volatile High
    # contributors blend. It is an OPERATIONAL BUFFER level (not the point
    # forecast): the stock cover needed to hit ~90% service on spiky,
    # high-priority items, independent of which model won champion selection.
    safety_stock_p90: Optional[pd.Series] = None
...
def _build_candidate_pool(profile_row: dict,
    ...
    # ── Per-segment exclusions ────────────────────────────────────────
    # A segment recipe may explicitly bar models from its pool (e.g. Volatile
    # Low contributors exclude Holt-Winters). Apply this LAST so it also
    # strips models re-added by the safety-net default shortlist above. The
    # profile-driven primary is never excluded, so the pool can't go empty.
    _excluded = set(arch.get('exclude') or [])
    if _excluded:
        candidates = [c for c in candidates
                      if c not in _excluded or c == auto_strat]
...
def forecast_one_sku(sku: str, panel: pd.DataFrame, profile_row: dict,
    ...
    # ════════════════════════════════════════════════════════════════
    # 🥇 PIPELINE STAGE 4 — FINAL CHAMPION = LOWEST TEST WMAPE
    # The earlier stages (champion-by-holdout, K-fold CV, weighted blend)
    # decide on leakage-shielded *validation*/CV WMAPE — deliberately NOT the
    # Test WMAPE shown in the comparison table. Planners found this confusing:
    # the model with the lowest *displayed* Test WMAPE could lose to the blend.
    # Per product decision, the FINAL pick now follows the lowest Test WMAPE
    # across every candidate AND the blend (which already carries a real
    # test_mape via the reconciliation above). Only runs when backtesting is on
    # (otherwise no Test WMAPE exists and we keep the routed/blended pick).
    # Caveat: the incumbent champion's test_mape may come from the leak-free
    # global-LGBM backtest path while pool candidates use a plain holdout (see
    # note above) — selecting across them is the intended, planner-requested
    # behavior, accepting the slight methodology mismatch.
    # ════════════════════════════════════════════════════════════════
    if run_backtest and all_algo_metrics:
        _ranked_by_test = sorted(
            ((s, m) for s, m in all_algo_metrics.items()
             if m.get('test_mape') is not None
             and np.isfinite(m.get('test_mape'))
             and m.get('future_forecast') is not None
             and len(m.get('future_forecast')) > 0),
            key=lambda kv: (
                kv[1]['test_mape'],
                kv[1].get('test_smape') if kv[1].get('test_smape') is not None else float('inf'),
                0 if kv[0] == strategy else 1,   # stability: keep incumbent on ties
                kv[0],
            ),
        )
        if _ranked_by_test:
            _winner_strat, _winner_m = _ranked_by_test[0]
            if _winner_strat != strategy:
                _prev_strategy, _prev_mape = strategy, mape
                strategy = _winner_strat
                forecast = _winner_m['future_forecast']
                mape = _winner_m.get('test_mape')
                smape = _winner_m.get('test_smape')
                bt_pred = _winner_m.get('test_pred')
                bt_actual = _winner_m.get('test_actual')
                ci = None  # pool candidate fits don't carry prediction intervals
                # Signed bias from the winner's held-out slice (+ve = over-forecast).
                try:
                    if bt_actual is not None and bt_pred is not None and len(bt_actual):
                        _sa = float(np.asarray(bt_actual, dtype=float).sum())
                        _sp = float(np.asarray(bt_pred, dtype=float).sum())
                        bias_pct = ((_sp - _sa) / _sa * 100.0) if abs(_sa) > 1e-9 else None
                    else:
                        bias_pct = None
                except Exception:
                    bias_pct = None
                # Re-apply MoM/YoY guardrails to the NEW forecast so the clamp
                # is never skipped just because the winning model changed. The
                # residual booster (Stage 2) was champion-specific and is NOT
                # re-run — we ship the winning candidate's forecast as-is.
                try:
                    if forecast is not None and len(history) >= 2:
                        forecast, _ = apply_business_rules(
                            history=history, forecast=forecast, freq=freq,
                            mom_tolerance=0.50, yoy_z=3.0)
                except Exception:
                    pass
                all_algo_metrics[strategy]['future_forecast'] = forecast
                _pick_note = (
                    f"final pick: {strategy} by lowest Test WMAPE ({mape:.1f}%) "
                    f"— overrode {_prev_strategy}"
                    + (f" ({_prev_mape:.1f}%)" if _prev_mape is not None else ""))
                notes = f"{notes} · {_pick_note}" if notes else _pick_note
    ...
    # ---- P90 safety-stock guardrail (Volatile High etc.) ----
    # When xgb_quantile_90 is in the candidate pool its 90th-percentile future
    # forecast is retained as a buffer level for safety-stock planning —
    # regardless of which model won the point-forecast champion selection.
    _p90_fc = (all_algo_metrics.get('xgb_quantile_90') or {}).get('future_forecast')
    safety_stock_p90 = (_p90_fc if (_p90_fc is not None and len(_p90_fc)) else None)
    ...
    return ForecastResult(sku=sku, strategy_used=strategy, forecast=forecast,
                          ...
                          safety_stock_p90=safety_stock_p90)
...
def apply_theme():
    ...
    # =================================================================
    # MODERN SAAS OVERRIDE LAYER
    # -----------------------------------------------------------------
    # Layered *after* the base theme so these rules win. Plain (non-f)
    # string — no brace-doubling needed. Goal: glassy rounded surfaces,
    # softer neutrals, a richer typographic scale, micro-interactions
    # (hover lift), a pill-style sidebar nav, and tidy scrollbars — the
    # look of a modern analytics SaaS rather than a Streamlit form.
    # =================================================================
    st.markdown("""
        <style>
        :root {
            ...
            --tl-radius: 16px;
            --tl-radius-sm: 10px;
            --tl-shadow: 0 1px 2px rgba(7,62,92,.05), 0 12px 28px -16px rgba(7,62,92,.22);
            --tl-shadow-hi: 0 2px 4px rgba(7,62,92,.06), 0 22px 44px -22px rgba(7,62,92,.30);
        }

        /* ===== App canvas — clean with a faint top-corner brand glows ===== */
        .stApp {
            background:
                radial-gradient(820px 380px at 8% -6%, rgba(7,62,92,.06), transparent 60%),
                radial-gradient(720px 360px at 100% -4%, rgba(239,118,2,.05), transparent 58%),
                linear-gradient(180deg, #f7f9fc 0%, #f3f6fa 100%) !important;
        }

        /* ===== Typographic scale ===== */
        h1 { font-size: 2.35rem !important; letter-spacing: -0.025em !important; }
        h2 { font-size: 1.6rem  !important; letter-spacing: -0.02em  !important; }
        h3 { font-size: 1.22rem !important; letter-spacing: -0.015em !important; }
        p, li, label, .stMarkdown { color: #334155; }

        /* ===== Glassy cards: metrics, expanders, dataframes ===== */
        [data-testid="stMetric"],
        [data-testid="stExpander"],
        [data-testid="stDataFrame"] {
        ...
        [data-testid="stMetric"] { position: relative; overflow: hidden; padding: 18px 22px 18px 26px !important; }
        /* Coloured left accent rail on metric cards */
        [data-testid="stMetric"]::before {
            content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 5px;
            background: linear-gradient(180deg, var(--tl-orange) 0%, var(--tl-orange-2) 100%);
            border-radius: 0 4px 4px 0;
        }
        [data-testid="stMetric"]:hover,
        [data-testid="stExpander"]:hover {
            box-shadow: var(--tl-shadow-hi) !important;
            transform: translateY(-2px);
            border-color: #dbe3ec !important;
        }
        [data-testid="stMetricValue"] { font-size: 1.9rem !important; letter-spacing: -0.02em; }

        [data-testid="stExpander"] summary { padding: 4px 2px; }
        [data-testid="stExpander"] summary:hover { color: var(--tl-orange) !important; }

        /* ===== Primary buttons — gradient + glow + lift ===== */
        .stButton > button {
            background: linear-gradient(135deg, var(--tl-orange) 0%, #f98a1f 100%) !important;
            border-radius: var(--tl-radius-sm) !important;
            font-weight: 650 !important;
            letter-spacing: .01em;
            box-shadow: 0 1px 2px rgba(239,118,2,.25), 0 10px 22px -12px rgba(239,118,2,.55) !important;
        }
        .stButton > button:hover {
            transform: translateY(-2px);
            box-shadow: 0 2px 4px rgba(239,118,2,.30), 0 16px 30px -12px rgba(239,118,2,.65) !important;
        }
        .stButton > button:active { transform: translateY(0); }
        .stDownloadButton > button {
            background: linear-gradient(135deg, var(--tl-blue) 0%, var(--tl-blue-2) 100%) !important;
            border-radius: var(--tl-radius-sm) !important;
            box-shadow: 0 10px 22px -14px rgba(7,62,92,.6) !important;
        }

        /* ===== Tabs — segmented pill bar, gradient active ===== */
        .stTabs [data-baseweb="tab-list"] {
        ...
        .stTabs [data-baseweb="tab"][aria-selected="true"] {
            background: linear-gradient(135deg, var(--tl-blue) 0%, var(--tl-blue-2) 100%) !important;
            box-shadow: 0 6px 16px -8px rgba(7,62,92,.55) !important;
        }
        ...
        div[data-baseweb="select"] > div:focus-within {
            ...
            box-shadow: 0 0 0 3px rgba(239,118,2,.12) !important;
        }
        ...
        /* ===== Pill-style nav radio (the #_nav_page radio in the sidebar) =====
           Hide the native radio dot and turn each option into a full-width
           card button; the checked option fills with the brand gradient.
           Uses :has() (supported in modern Chromium / Streamlit 1.3x). */
        [data-testid="stSidebar"] div[role="radiogroup"] {
            gap: 8px !important; display: flex; flex-direction: column;
        }
        [data-testid="stSidebar"] div[role="radiogroup"] > label {
            ...
            margin: 0 !important;
            box-shadow: 0 1px 2px rgba(7,62,92,.04);
        ...
        [data-testid="stSidebar"] div[role="radiogroup"] > label:hover {
            ...
            box-shadow: 0 6px 16px -10px rgba(7,62,92,.4);
        }
        /* Hide the circular radio marker, keep the text label */
        [data-testid="stSidebar"] div[role="radiogroup"] > label > div:first-child {
            display: none !important;
        }
        /* Active state */
        [data-testid="stSidebar"] div[role="radiogroup"] > label:has(input:checked) {
            background: linear-gradient(135deg, var(--tl-blue) 0%, var(--tl-blue-2) 100%);
            border-color: transparent;
            box-shadow: 0 10px 24px -12px rgba(7,62,92,.6);
        }
        ...
        /* ===== Section-header banner + KPI hero (custom helpers) ===== */
        .tl-banner {
            display:flex; align-items:center; gap:18px;
            background:
                radial-gradient(120% 140% at 0% 0%, rgba(239,118,2,.18), transparent 55%),
                linear-gradient(135deg, var(--tl-blue) 0%, var(--tl-blue-2) 100%);
            color:#fff; padding:20px 26px; border-radius:18px;
            margin: 2px 0 20px 0;
            box-shadow: 0 18px 40px -22px rgba(7,62,92,.55);
            position: relative; overflow: hidden;
        }
        .tl-banner::after {
            content:''; position:absolute; right:-40px; top:-40px;
            width:180px; height:180px; border-radius:50%;
            background: radial-gradient(circle, rgba(255,165,82,.30), transparent 70%);
        }
        .tl-banner .tl-eyebrow {
            font-size:.78rem; text-transform:uppercase; letter-spacing:.1em;
            font-weight:700; opacity:.85;
        }
        .tl-banner .tl-title { font-size:1.5rem; font-weight:800; letter-spacing:-.02em; margin-top:2px; }
        .tl-banner .tl-sub { font-size:.92rem; opacity:.85; margin-top:3px; }
        .tl-banner .tl-icon {
            font-size:1.9rem; background:rgba(255,255,255,.14);
            width:54px; height:54px; border-radius:14px;
            display:flex; align-items:center; justify-content:center; flex:0 0 auto;
        }

        .tl-kpi-row { display:grid; grid-template-columns: repeat(auto-fit, minmax(150px,1fr)); gap:14px; margin: 4px 0 22px 0; }
        .tl-kpi {
            background: linear-gradient(180deg,#ffffff 0%,#fbfdff 100%);
            border:1px solid var(--tl-border); border-radius:16px;
            padding:16px 18px; position:relative; overflow:hidden;
            box-shadow: var(--tl-shadow); transition: all .18s ease;
        }
        .tl-kpi:hover { transform:translateY(-3px); box-shadow: var(--tl-shadow-hi); }
        .tl-kpi::before {
            content:''; position:absolute; left:0; top:0; bottom:0; width:4px;
            background: linear-gradient(180deg, var(--accent,#073e5c), var(--accent2,#0a527a));
        }
        .tl-kpi .k-ico { font-size:1.15rem; }
        .tl-kpi .k-label { color:var(--tl-muted); font-size:.72rem; text-transform:uppercase; letter-spacing:.06em; font-weight:700; margin-top:6px; }
        .tl-kpi .k-value { color:var(--tl-blue); font-size:1.7rem; font-weight:800; letter-spacing:-.02em; line-height:1.05; margin-top:2px; }
        .tl-kpi .k-sub { color:var(--tl-muted); font-size:.74rem; margin-top:3px; }

        /* ===== Tidy scrollbars ===== */
        ::-webkit-scrollbar { width:10px; height:10px; }
        ::-webkit-scrollbar-thumb { background:#c7d2dd; border-radius:8px; border:2px solid transparent; background-clip:content-box; }
        ::-webkit-scrollbar-thumb:hover { background:#9fb0c0; border:2px solid transparent; background-clip:content-box; }
        ::-webkit-scrollbar-track { background:transparent; }
        </style>
    """, unsafe_allow_html=True)

    # ---- Brand Plotly template so charts match the SaaS look -----------
    # Registered + set as default; figures that don't hard-code a template
    # (most update_layout calls) inherit Inter typography, a brand colorway
    # and soft gridlines. Charts that explicitly pass template='plotly_white'
    # keep their own clean look.
    try:
        import plotly.io as _pio
        import plotly.graph_objects as _go
        if 'timelens' not in _pio.templates:
            _tl_tpl = _go.layout.Template()
            _tl_tpl.layout = _go.Layout(
                font=dict(family="Inter, -apple-system, Segoe UI, sans-serif",
                          color="#0f2b3d", size=13),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                colorway=["#073e5c", "#ef7602", "#0a9396", "#9b5de5",
                          "#2a9d8f", "#e76f51", "#457b9d", "#f4a261"],
                title=dict(font=dict(size=16, color="#073e5c")),
                xaxis=dict(gridcolor="#eef2f6", zerolinecolor="#e2e8f0",
                           linecolor="#e2e8f0", ticks="outside",
                           tickcolor="#cbd5e1"),
                yaxis=dict(gridcolor="#eef2f6", zerolinecolor="#e2e8f0",
                           linecolor="#e2e8f0"),
                legend=dict(bgcolor="rgba(255,255,255,.6)", bordercolor="#e8edf2",
                            borderwidth=1),
                hoverlabel=dict(font=dict(family="Inter, sans-serif", size=12),
                                bgcolor="#073e5c", bordercolor="#073e5c",
                                font_color="#ffffff"),
                margin=dict(t=54, r=20, b=44, l=56),
            )
            _pio.templates["timelens"] = _tl_tpl
            # Compose with plotly_white so we keep its sensible base defaults.
            _pio.templates.default = "plotly_white+timelens"
    except Exception:
        pass
...
def tl_section_header(title: str, subtitle: str = "", eyebrow: str = "",
                      icon: str = ""):
    """Render the standard gradient section banner used at the top of a tab.

    Gives every tab the same modern, branded header so the app reads as a
    designed dashboard rather than a stack of forms. `icon` is any emoji.
    """
    icon_html = f"<div class='tl-icon'>{icon}</div>" if icon else ""
    eyebrow_html = (f"<div class='tl-eyebrow'>{eyebrow}</div>" if eyebrow else "")
    sub_html = f"<div class='tl-sub'>{subtitle}</div>" if subtitle else ""
    st.markdown(
        f"<div class='tl-banner'>{icon_html}"
        f"<div>{eyebrow_html}<div class='tl-title'>{title}</div>{sub_html}</div>"
        f"</div>",
        unsafe_allow_html=True,
    )


def tl_kpi_row(cards):
    """Render a responsive KPI hero strip.

    `cards` is a list of dicts: {label, value, sub?, icon?, accent?, accent2?}.
    Pure HTML/CSS (one node) so it lays out as a clean auto-fit grid rather
    than fighting Streamlit column widths.
    """
    cells = []
    for c in cards:
        accent = c.get('accent', '#073e5c')
        accent2 = c.get('accent2', accent)
        ico = f"<div class='k-ico'>{c['icon']}</div>" if c.get('icon') else ""
        sub = f"<div class='k-sub'>{c['sub']}</div>" if c.get('sub') else ""
        cells.append(
            f"<div class='tl-kpi' style='--accent:{accent};--accent2:{accent2};'>"
            f"{ico}<div class='k-label'>{c.get('label','')}</div>"
            f"<div class='k-value'>{c.get('value','')}</div>{sub}</div>"
        )
    st.markdown(f"<div class='tl-kpi-row'>{''.join(cells)}</div>",
                unsafe_allow_html=True)
...
def render_config():
    """Full input + configuration form for the dedicated 'Setup &
    Configuration' page.

    Renders into the MAIN content area (its own window) rather than the
    sidebar, and returns the cfg dict (or None if no dataset has been
    loaded yet). The returned cfg is cached by the caller into
    st.session_state['app_cfg'] so the workflow tabs can consume it
    without the config widgets needing to be on-screen.
    """
    with st.container():
        # ------------------------------------------------------------
        ...
        with st.expander("2. Column Configuration", expanded=True):
            ...
            # ── Forecast level (aggregation grain) ──────────────────────
            # By default each SKU is forecast on its own. Planners often want
            # the forecast at a coarser grain — a region total, a category,
            # any combination, or one portfolio-wide number. We aggregate the
            # history to that grain (sum sales, mean prices) and forecast the
            # aggregated series directly with the same engine.
            st.markdown("**Forecast level**")
            _level_choice = st.radio(
                "Aggregation grain",
                ["Per-SKU (most granular)", "Custom group level", "Overall total"],
                key='forecast_level_choice',
                label_visibility='collapsed',
                help="Per-SKU forecasts every item. Custom aggregates sales to "
                     "the column(s) you pick (e.g. region, category, or a "
                     "combination) and forecasts each group directly. Overall "
                     "rolls everything into one portfolio-wide series.",
            )
            forecast_level_mode = 'sku'
            forecast_level_cols: List[str] = []
            if _level_choice == "Custom group level":
                forecast_level_mode = 'custom'
                _grp_candidates = [c for c in cols
                                   if c not in (date_col, sales_col, sku_col)]
                # NOTE: `key` alone persists the selection across reruns. Do
                # NOT also pass `default=` reading the same session_state key —
                # Streamlit raises StreamlitAPIException for that conflict,
                # which would abort the whole sidebar render.
                forecast_level_cols = st.multiselect(
                    "Group by column(s)",
                    _grp_candidates,
                    key='forecast_level_cols_sel',
                    help="Pick one column (e.g. region) or several (e.g. "
                         "region + category) — one forecast is produced per "
                         "unique combination.",
                )
                if not forecast_level_cols:
                    st.info("Pick at least one column to group by — "
                            "until then the engine forecasts per SKU.")
                    forecast_level_mode = 'sku'
                else:
                    try:
                        _n_ent = (df[forecast_level_cols].astype(str)
                                  .agg(' | '.join, axis=1).nunique())
                        st.caption(
                            f"→ **{_n_ent:,}** "
                            f"{' × '.join(forecast_level_cols)} series will be "
                            f"forecast (sales summed, prices averaged per group)."
                        )
                    except Exception:
                        pass
            elif _level_choice == "Overall total":
                forecast_level_mode = 'overall'
                st.caption("→ **1** portfolio-wide series (all items summed).")
        ...
        with st.expander("3b. Top-Down Forecasting  ·  for new / sparse / noisy SKUs",
                         expanded=False):
            st.caption(
                "When a SKU is **new, has little history, or is noisy/intermittent**, "
                "its own bottom-up model has too little signal to be accurate. "
                "Top-down instead forecasts a **cleaner aggregate** you choose "
                "(e.g. brand or category total) and splits it back down to the SKU "
                "by its share — usually far more reliable for these hard cases."
            )
            td_enabled = st.checkbox(
                "Enable top-down for hard-to-forecast SKUs",
                value=False,
                help="Off by default. When on, qualifying SKUs are re-forecast "
                     "top-down after the normal per-SKU pass; all other SKUs are "
                     "unaffected.",
            )

            # Candidate aggregation levels = any column that isn't the date,
            # target, or SKU id. Seed a sensible default the first time only
            # (avoids the key+default Streamlit conflict).
            _td_level_candidates = [c for c in cols
                                    if c not in (date_col, sales_col, sku_col)]
            if 'td_levels_sel' not in st.session_state:
                st.session_state['td_levels_sel'] = [
                    c for c in ('brand', 'category', 'segment')
                    if c in _td_level_candidates][:1]
            td_levels = st.multiselect(
                "Aggregate to level(s)",
                _td_level_candidates,
                key='td_levels_sel',
                help="Pick the level whose total is well-behaved — usually brand "
                     "or category. Choose several to aggregate at a finer node "
                     "(e.g. brand × category).",
                disabled=not td_enabled,
            )
            td_apply_to = st.multiselect(
                "Apply top-down to which SKUs?",
                ["New / cold-start SKUs", "Short-history SKUs",
                 "Lumpy / intermittent SKUs", "Noisy (high variability) SKUs"],
                default=["New / cold-start SKUs", "Lumpy / intermittent SKUs"],
                help="Only SKUs matching these profiles are re-routed top-down; "
                     "everything else keeps its per-SKU forecast.",
                disabled=not td_enabled,
            )
            td_disagg = st.selectbox(
                "How to split the aggregate back to each SKU",
                ["Historical average share", "Recent share (last 6 periods)",
                 "Equal share within group"],
                help="Historical / Recent use each SKU's past share of the group "
                     "total. Equal share suits brand-new SKUs with no history.",
                disabled=not td_enabled,
            )
            if td_enabled and not td_levels:
                st.warning("Pick at least one aggregation level for top-down "
                           "to run.")
            elif td_enabled:
                st.success(
                    f"Top-down ON → {' × '.join(td_levels)} aggregate, "
                    f"split by {td_disagg.lower()}."
                )
            # Normalise the SKU-trigger multiselect to flags for the engine.
            _td_apply = {
                'cold':  "New / cold-start SKUs" in td_apply_to,
                'short': "Short-history SKUs" in td_apply_to,
                'lumpy': "Lumpy / intermittent SKUs" in td_apply_to,
                'noisy': "Noisy (high variability) SKUs" in td_apply_to,
            }
        ...
        # NOTE: `sku_col`/`brand_col`/`segment_col` here are always the ORIGINAL
        # per-item columns — the early tabs (Load Data, EDA) operate on raw rows
        # and need them. The forecast LEVEL is applied only inside the
        # profiling+forecast pipeline via resolve_pipeline_cfg(), which swaps in
        # the aggregated entity key and drops the per-item hierarchies.
        return {
            'date_col': date_col, 'date_format': date_format or None,
            'sku_col': sku_col, 'sales_col': sales_col,
            'forecast_level_mode': forecast_level_mode,
            'forecast_level_cols': list(forecast_level_cols),
            'segment_col': _resolved_seg,
            'brand_col': _resolved_brand,
            'freq': freq, 'horizon': horizon,
            'cold_thr': cold_thr, 'short_thr': short_thr,
            # Top-down forecasting (3b) — re-route new/sparse/noisy SKUs to a
            # disaggregated aggregate forecast. Disabled by default.
            'top_down_enabled': bool(td_enabled),
            'top_down_levels': list(td_levels),
            'top_down_apply': _td_apply,
            'top_down_disagg': td_disagg,
...
def render_profiling_tab(cfg):
    ...
    # From here on the pipeline runs at the chosen forecast grain (no-op for
    # per-SKU): sku_col becomes the aggregated entity key.
    cfg = resolve_pipeline_cfg(cfg)
    st.markdown(f"""
    ...
    _cache_key = (
        ...
        cfg.get('date_format'),
        _hist_start,
        # Re-aggregate when the planner changes the forecast grain.
        cfg.get('forecast_level_mode', 'sku'),
        tuple(cfg.get('forecast_level_cols') or []),
    )
    if st.session_state.get('_profile_df_cache_key') != _cache_key:
        ...
        # Collapse to the chosen forecast grain (no-op for per-SKU). Dates are
        # already parsed above, so the entity rows aggregate cleanly per period.
        df = aggregate_to_forecast_level(df, cfg)
    ...
    # =================================================================
    # The retail segmentation sub-step is a per-SKU concept that writes a
    # 'segment' column back into df_raw. At coarser forecast grains the entity
    # key doesn't exist in df_raw, so we skip this UI and let profiling derive
    # the demand classification directly from each aggregated series below.
    _level_mode = cfg.get('forecast_level_mode', 'sku')
    if _level_mode == 'sku':
        ...
        _lvl_lbl = _level_entity_label(cfg)
        st.info(
            f"📊 **Forecast level: {_lvl_lbl}.** History is aggregated to "
            f"**{df[cfg['sku_col']].nunique():,}** {_lvl_lbl} series "
            f"(sales summed, prices averaged). Per-SKU segmentation is skipped "
            f"at this grain — each aggregated series is classified directly "
            f"below."
        )
    st.markdown("---")
    ...
    if not st.session_state.get('profiled'):
        if st.button("Run intermittency profiling", use_container_width=True, key='btn_profile'):
            _lvl_lbl = _level_entity_label(cfg)
            with st.spinner(f"Profiling {df[cfg['sku_col']].nunique():,} {_lvl_lbl} series…"):
                ...
                # profile_all_skus recompute on the filtered panel. At non-SKU
                # grains the cached stats are keyed by SKU and never match the
                # aggregated entity keys, so we force a fresh recompute.
                _seg_stats = st.session_state.get('retail_seg_df')
                if (st.session_state.get('_profile_filter_dropped', 0)
                        or cfg.get('forecast_level_mode', 'sku') != 'sku'):
                    _seg_stats = None
...
def render_forecast_tab(cfg):
    ...
    # Run at the chosen forecast grain (no-op for per-SKU). This flows into
    # run_forecasts() and render_results() so the whole tab is consistent.
    cfg = resolve_pipeline_cfg(cfg)
    ...
    # Forecast grain — 'SKU' by default, else the aggregated entity label.
    _lvl = _level_entity_label(cfg)
    _is_sku_level = cfg.get('forecast_level_mode', 'sku') == 'sku'
    _ent = "SKU" if _is_sku_level else _lvl
    if not _is_sku_level:
        st.caption(f"📊 Forecasting at the **{_lvl}** level "
                   f"({profiles['sku'].nunique():,} aggregated series).")

    # Entity selection — sample by default to keep things responsive
    selection_mode = st.radio(
        "What to forecast",
        [f"Pick specific {_ent}s", "Sample N per strategy", f"All {_ent}s (slow)"],
        horizontal=True,
    )

    if selection_mode == f"Pick specific {_ent}s":
        ...
        st.caption(f"{len(_sku_opts):,} {_ent}(s) match the filter.")
        skus = st.multiselect(f"{_ent}s", _sku_opts,
                              default=list(_sku_opts[:5]))
    elif selection_mode == "Sample N per strategy":
        ...
        if len(skus) > 200:
            st.warning(f"Forecasting {len(skus):,} {_ent} series may take several minutes.")
    ...
    # ── Fast mode: cohort-scoped global training ───────────────────────
    # The global LightGBM trains on the WHOLE catalog by default — for a
    # small selection that fixed cost dominates the run. Fast mode trains it
    # only on the selected SKUs + their brand/segment peers (still enough to
    # borrow cross-series strength) which is dramatically faster on small
    # picks. Default ON for small selections, OFF for big ones (where the
    # full-catalog pool is both affordable and most beneficial).
    _FAST_MODE_SKU_THRESHOLD = 25
    fast_mode = st.checkbox(
        "⚡ Fast mode — train the global model on selected SKUs + their "
        "brand/segment peers (faster on small selections)",
        value=(use_global and len(skus) <= _FAST_MODE_SKU_THRESHOLD),
        disabled=not use_global,
        help="Instead of training the pooled global LightGBM on the entire "
             "catalog, restrict it to the selected SKUs plus every SKU sharing "
             "their brand or segment. Big speedup for small runs; forecast "
             "numbers can shift slightly vs full-catalog pooling. Turn OFF to "
             "reproduce the exact full-catalog forecasts.",
    )
    ...
    treat_outliers = st.checkbox(
        "🧹 Clean outliers before training (one consistent policy for every model)",
        value=True,
        help="Caps unexplained demand spikes/dips back onto a robust "
             "median ± k×IQR fence (per series) BEFORE lags, the global "
             "LightGBM, and all statistical models train — so every algorithm "
             "sees the same cleaned history. Months flagged by an active "
             "promo / festival / holiday / planned-event / price-change column "
             "are EXCLUDED from cleaning, so genuine event lifts are preserved "
             "for the models to learn.",
    )
    if treat_outliers:
        outlier_k = st.slider(
            "Outlier sensitivity — × IQR from the median",
            min_value=2.0, max_value=5.0, value=3.0, step=0.5,
            help="Lower = more aggressive cleaning (more months treated as "
                 "outliers). 3.0 is a conservative default.",
        )
    else:
        outlier_k = 3.0
    ...
    if st.button("Run forecasts", type='primary', use_container_width=True):
        ...
        run_forecasts(cfg, df, profiles, skus, use_global, enable_chronos,
                      ...
                      treat_outliers=treat_outliers, outlier_k_iqr=outlier_k,
                      fast_mode=fast_mode)
...
def run_forecasts(cfg, df, profiles, skus, use_global, enable_chronos, reconcile,
                  ...
                  compare_algos: Optional[List[str]] = None,
                  treat_outliers: bool = True, outlier_k_iqr: float = 3.0,
    ...
    with st.status("🚀 Running forecast pipeline…", expanded=True) as status:
        ...
        # PERF: the full-catalog panel build is expensive and was repeated on
        # every "Run forecasts" click even when the underlying data and
        # outlier policy were unchanged (e.g. re-running with a different SKU
        # selection). Cache it in session_state keyed by a CHEAP signature of
        # the inputs that actually affect the panel — so re-runs on the same
        # data skip Step 2 entirely. The signature deliberately excludes the
        # SKU selection (the panel is whole-catalog regardless of selection).
        try:
            _date_min = str(pd.to_datetime(df_enriched[cfg['date_col']]).min())
            _date_max = str(pd.to_datetime(df_enriched[cfg['date_col']]).max())
        except Exception:
            _date_min = _date_max = ''
        panel_sig = (
            len(df_enriched), tuple(df_enriched.columns), _date_min, _date_max,
            round(float(pd.to_numeric(df_enriched[cfg['sales_col']],
                                      errors='coerce').fillna(0).sum()), 2),
            cfg['freq'], bool(treat_outliers), float(outlier_k_iqr),
            tuple(event_cols), tuple(user_exog_numeric), tuple(user_exog_categorical),
            cfg['date_col'], cfg['sales_col'], cfg['sku_col'],
            cfg['brand_col'], cfg['segment_col'],
        )
        _cache = st.session_state.get('_panel_cache')
        if _cache is not None and _cache.get('sig') == panel_sig:
            panel = _cache['panel']
            _osum = _cache.get('outlier_summary')
            st.write(f"✓ Reused cached panel: **{len(panel):,} rows × "
                     f"{len(panel.columns)} features** (unchanged data — skipped rebuild)")
        else:
            # Planned-event flags (evt_*) are always treated as explanatory so a
            # known event month is never scrubbed as a statistical outlier.
            panel = build_panel_features(
                ...
                outlier_k_iqr=outlier_k_iqr,
                outlier_explain_extra=event_cols + user_exog_numeric,
            )
            _osum = getattr(build_panel_features, 'last_outlier_summary', None)
            st.session_state['_panel_cache'] = {
                'sig': panel_sig, 'panel': panel, 'outlier_summary': _osum,
            }
            st.write(f"✓ Built panel: **{len(panel):,} rows × {len(panel.columns)} features**")
        if treat_outliers and _osum:
            st.write(
                f"✓ Outlier policy (±{_osum['k_iqr']:g}×IQR): cleaned "
                f"**{_osum['treated']:,}** unexplained spike(s); kept "
                f"**{_osum['event_kept']:,}** event/promo-flagged month(s) "
                f"intact across {len(_osum['explain_cols'])} driver column(s)."
            )
        ...
        # ── Cohort scoping for global training (Fast mode) ───────────────
        # The pooled global LightGBM is the dominant fixed cost on small runs.
        # When fast_mode is on, train it on just the selected SKUs + every SKU
        # sharing their brand/segment (still enough peers to borrow strength),
        # instead of the entire catalog. Per-SKU forecasting still iterates
        # only `skus`. `global_panel` is what the two global models train on.
        sku_col = cfg['sku_col']
        global_panel = panel
        cohort_note = ''
        if use_global and fast_mode:
            cohort_skus = set(skus)
            _psel = profiles[profiles['sku'].isin(skus)]
            if cfg['brand_col'] and 'brand' in profiles.columns:
                _brands = set(_psel['brand'].dropna().astype(str))
                cohort_skus |= set(
                    profiles[profiles['brand'].astype(str).isin(_brands)]['sku'])
            if cfg['segment_col'] and 'segment' in profiles.columns:
                _segs = set(_psel['segment'].dropna().astype(str))
                cohort_skus |= set(
                    profiles[profiles['segment'].astype(str).isin(_segs)]['sku'])
            _scoped = panel[panel[sku_col].isin(cohort_skus)]
            # Only bother if scoping actually trims the panel meaningfully.
            if 0 < len(_scoped) < len(panel):
                global_panel = _scoped.reset_index(drop=True)
                cohort_note = (
                    f"⚡ Fast mode: global model trained on **{global_panel[sku_col].nunique():,}** "
                    f"cohort SKU(s) ({len(global_panel):,} rows) vs "
                    f"**{panel[sku_col].nunique():,}** total.")
                st.write(cohort_note)

        # Cheap cache key for train_global_lightgbm (see its docstring): a
        # signature of the data the global model actually trains on, so
        # @st.cache_resource keys on this string instead of hashing the frame.
        global_sig = (
            f"{panel_sig}|fast={int(bool(fast_mode))}|"
            f"rows={len(global_panel)}|skus={global_panel[sku_col].nunique()}|"
            f"cats={tuple(cats)}")

        # ── Steps 3 & 4 · Train global LightGBM models (in parallel) ─────
        # Step 3 = forecast model (no holdout); Step 4 = leak-free backtest
        # model (last `horizon` periods removed PER SKU). The two are fully
        # independent and LightGBM's .fit() releases the GIL, so we train them
        # concurrently rather than back-to-back — roughly halving the global
        # training wall-clock, which is the dominant fixed cost on small runs.
        #
        # Backtest model holds out the FULL forecast horizon per SKU (not just
        # 1 month) so the Test WMAPE measures error over the same horizon we
        # ship to production. Per-SKU the actual test window may be shorter
        # (short-history SKUs are capped at half their history by
        # `_smart_test_horizon`); excluding the full horizon here is the safe
        # upper bound, so no test month can ever leak into this model.
        global_pkg = None
        global_pkg_backtest = None
        if use_global:
            _need_bt = bool(run_backtest)
            status.update(label="🌲 Steps 3–4/6 · Training global LightGBM models (parallel)…")
            live.markdown(
                "One pooled forecast model + one leak-free backtest model "
                "(last horizon removed per SKU), trained **concurrently** — "
                "borrows strength via brand/segment/price-band embeddings.")
            _t = time.time()

            def _train_fc():
                return train_global_lightgbm(
                    global_panel, cfg['sku_col'], cfg['date_col'], cfg['sales_col'],
                    cfg['freq'], cats, holdout_periods=0,
                    cache_key=f"{global_sig}|hold=0")

            def _train_bt():
                return train_global_lightgbm(
                    global_panel, cfg['sku_col'], cfg['date_col'], cfg['sales_col'],
                    cfg['freq'], cats, holdout_periods=horizon,
                    cache_key=f"{global_sig}|hold={horizon}")

            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as _gex:
                _fc_future = _gex.submit(_train_fc)
                _bt_future = _gex.submit(_train_bt) if _need_bt else None
                global_pkg = _fc_future.result()
                global_pkg_backtest = _bt_future.result() if _bt_future else None

            if global_pkg:
                st.write(f"✓ Forecast LGBM trained "
                         f"({len(global_panel):,} rows × {len(global_pkg.feature_cols)} features)")
            else:
                st.warning("LightGBM unavailable — SKUs needing it will fall back to Holt-Winters.")
            if global_pkg_backtest is not None:
                kept = len(global_pkg_backtest.panel_history)
                st.write(f"✓ Backtest LGBM trained "
                         f"(last {horizon} period(s) excluded, {kept:,} rows kept)")
            st.write(f"✓ Global models ready in **{_fmt_dur(time.time()-_t)}** (parallel)")
        ...
        # ── Step 6 · Top-down routing for new / sparse / noisy SKUs ─────
        # Replace the bottom-up forecast of hard-to-forecast SKUs with a
        # disaggregated aggregate forecast (group total × SKU share). Runs
        # BEFORE brand reconciliation so the final forecasts are the ones
        # reconciled to brand totals. No-op unless enabled in config.
        if cfg.get('top_down_enabled'):
            status.update(label="🔺 Step 6/6 · Top-down forecasting for new / sparse / noisy SKUs…")
            live.markdown("Forecasting cleaner aggregates and splitting them back to hard-to-forecast SKUs.")
            try:
                results, td_summary = apply_top_down_routing(
                    results, profiles, df, cfg, horizon)
                st.session_state.top_down_summary = td_summary
                if td_summary.get('enabled'):
                    _rs = ', '.join(f"{k}: {v}" for k, v in
                                    td_summary.get('reasons', {}).items()) or 'no SKUs matched'
                    st.write(
                        f"✓ Top-down applied to **{td_summary['n_rerouted']}** SKU(s) "
                        f"across **{td_summary['n_groups']}** "
                        f"{' × '.join(td_summary['levels'])} group(s) ({_rs})")
                elif td_summary.get('note'):
                    st.write(f"ℹ️ Top-down skipped — {td_summary['note']}")
            except Exception as e:
                st.warning(f"Top-down step skipped: {e}")
...
def _build_summary_frame(results: List[ForecastResult]) -> pd.DataFrame:
    ...
    for r in results:
        ...
        # P90 safety-stock guardrail — present when xgb_quantile_90 is a
        # candidate (Volatile High contributors). `p90_safety_stock` is the
        # 90th-percentile cover total; `p90_buffer_vs_point` is the extra units
        # above the point forecast needed to hit ~90% service on these spiky,
        # high-priority items. NaN for SKUs without the quantile model.
        _p90 = getattr(r, 'safety_stock_p90', None)
        if _p90 is not None and len(_p90):
            _p90_total = float(_p90.sum())
            _pt_total = float(r.forecast.sum()) if r.forecast is not None else 0.0
            row['p90_safety_stock'] = _p90_total
            row['p90_buffer_vs_point'] = _p90_total - _pt_total
        else:
            row['p90_safety_stock'] = np.nan
            row['p90_buffer_vs_point'] = np.nan
...
def render_results(cfg):
    ...
    # P90 safety-stock guardrail — surface the xgb_quantile_90 buffer columns
    # with friendly labels, or hide them when no SKU carries a P90 (i.e. no
    # Volatile High items / the quantile model wasn't in any candidate pool).
    _disp, _col_cfg = summary, {}
    _has_p90 = ('p90_safety_stock' in summary.columns
                and summary['p90_safety_stock'].notna().any())
    if _has_p90:
        _n_p90 = int(summary['p90_safety_stock'].notna().sum())
        _tot_buf = float(summary['p90_buffer_vs_point'].fillna(0).sum())
        st.markdown(
            f"<div style='padding:10px 14px;background:#ef76021A;border-radius:8px;"
            f"border-left:4px solid {DHISHAAI_ORANGE};margin-bottom:8px;'>"
            f"🛡️ <b>P90 safety-stock guardrail</b> active on <b>{_n_p90:,}</b> "
            f"high-priority SKU(s) (xgb_quantile_90 in the blend — e.g. Volatile "
            f"High contributors). Total recommended buffer above the point "
            f"forecast: <b>{_tot_buf:,.0f} units</b>.</div>",
            unsafe_allow_html=True,
        )
        _col_cfg = {
            'p90_safety_stock': st.column_config.NumberColumn(
                'P90 safety stock', format='%.0f',
                help='90th-percentile cover (xgb_quantile_90) — the stock needed '
                     'to hit ~90% service on spiky, high-priority items, kept '
                     'regardless of which model won the point-forecast champion.'),
            'p90_buffer_vs_point': st.column_config.NumberColumn(
                'Buffer vs point', format='%+.0f',
                help='P90 − champion point forecast: the extra units to carry as '
                     'a safety buffer on these items.'),
        }
    else:
        _disp = summary.drop(
            columns=[c for c in ('p90_safety_stock', 'p90_buffer_vs_point')
                     if c in summary.columns], errors='ignore')

    st.dataframe(_disp, use_container_width=True, height=300, column_config=_col_cfg)
    ...
    # Train/test boundary — the held-out test window starts here. The in-sample
    # fit (training) is CLIPPED to strictly before this point so the training
    # (green) and testing (purple) segments never sit on the same dates. This
    # removes the train/test overlap that made the chart hard to read.
    ...
    if tr_pred_series is not None and len(tr_pred_series) > 0:
        ...
        if _test_start is not None:
            _tr = _tr[_tr.index < _test_start]   # keep training region only
        if len(_tr) > 0:
            fig.add_trace(go.Scatter(
                x=list(_tr.index),
                y=list(_tr.values),
            ...
            last_bridge = (_tr.index[-1], float(_tr.values[-1]))
    ...
    # Training-zone label — anchors the left (untinted) region so the three
    # zones (training → backtest → forecast) are each named.
    if last_actual_date is not None and len(history):
        _train_end = _test_start if _test_start is not None else last_actual_date
        _train_dates = [d for d in history.index if d < _train_end]
        if _train_dates:
            _trmid = _train_dates[len(_train_dates) // 2]
            fig.add_annotation(
                x=_trmid, y=1.06, xref='x', yref='paper',
                text="Training", showarrow=False,
                font=dict(size=11, color=DHISHAAI_BLUE,
                          family='Inter, system-ui, sans-serif'),
                opacity=0.7,
            )

    # Backtest (test-holdout) zone — its own soft purple tint + label so the
    # chart reads as three clean zones: training → backtest → forecast.
    if (_test_start is not None and last_actual_date is not None
            and _test_start != last_actual_date):
        fig.add_vrect(
            x0=_test_start, x1=last_actual_date,
            fillcolor='rgba(124, 58, 237, 0.05)',
            line_width=0, layer='below',
        )
        fig.add_vline(
            x=_test_start,
            line=dict(color=TEST_COLOR, width=1.2, dash='dot'),
            opacity=0.55,
        )
        # Midpoint of the test window for the floating label.
        _test_dates = [d for d in history.index if _test_start <= d <= last_actual_date]
        if _test_dates:
            _tmid = _test_dates[len(_test_dates) // 2]
            fig.add_annotation(
                x=_tmid, y=1.06, xref='x', yref='paper',
                text="Backtest", showarrow=False,
                font=dict(size=11, color=TEST_COLOR,
                          family='Inter, system-ui, sans-serif'),
                opacity=0.85,
            )
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(0,0,0,0)',
        height=520,
        margin=dict(l=20, r=20, t=96, b=80),
        hovermode='x unified',
        hoverlabel=dict(
...
# -----------------------------------------------------------------
SEGMENT_DB_PATH = Path(__file__).parent / "dhisha_segments.db"


def _segment_db_connect() -> sqlite3.Connection:
    """Open (and create-if-needed) the segments DB. Idempotent."""
    conn = sqlite3.connect(str(SEGMENT_DB_PATH))
    conn.executescript("""
...
def build_causal_graph(treatments: List[str], outcome: str, confounders: List[str],
    ...
    confounders (common causes) → both treatment and outcome.
    instruments → treatment only (used for IV identification).
    effect_modifiers → outcome only (heterogeneity, not a confounder).
    """
    if graphviz is None:
        return ""
    ...
    if st.button("Run EDA", use_container_width=True, key='eda_run'):
        try:
            with st.spinner("Building EDA..."):
                ...
                # Standalone single-series EDA has no Input-config exog — clear
                # any stale exog frame from a prior forecast-tab EDA run.
                st.session_state['causal_exog_df'] = None
...
def _causal_interpretation(treatment, outcome, theta, features_df):
    """Turn a causal coefficient into a plain-language sentence + an elasticity.

    Returns (interpretation_str, elasticity_or_nan). The elasticity is the
    arc elasticity at the data means — the % change in demand per +1% change
    in the lever — which is the form business stakeholders reason about. It's
    only meaningful for continuous levers with a non-zero mean, so flags like
    promo_flag return NaN (their coefficient is already a per-event lift).
    """
    ...
    try:
        ...
        # Only continuous levers (more than two distinct values) get an
        # elasticity; binary flags don't have a meaningful "1%" change.
...
def _causal_robustness_verdict(refutation, theta):
    """Compare the refuted effect (random common cause) to the original.

    Returns (verdict_str, new_effect). A robust causal effect should barely
    move when a random confounder is injected.
    """
    new_effect = np.nan
    try:
        new_effect = float(getattr(refutation, 'new_effect', np.nan))
    except Exception:
        new_effect = np.nan
    ...
    if pct < 0.20:
        return "✓ Robust", new_effect
    if pct < 0.50:
        return "~ Moderate", new_effect
    return "▲ Caution", new_effect


# ── Full DoWhy support: estimator catalog + refuter battery ───────────
# DoWhy's 4-step workflow is model → identify → estimate → refute. The tab
# now exposes the full built-in surface (no extra deps beyond dowhy/graphviz):
# multiple estimators, the identified estimand, a refuter battery, and CIs.

def _causal_estimator_catalog(features_df, treatment: str) -> List[Tuple[str, str, bool]]:
    """Return [(method_name, human_label, applicable)] for this treatment.

    Propensity-score / matching estimators assume a BINARY treatment, so they
    are only marked applicable when the treatment has ≤ 2 distinct values.
    """
    try:
...
def _estimate_one_method(m, est, method_name: str):
    ...
    Supplies the method_params a few estimators require so they don't crash on
    defaults; everything is wrapped so a failing method degrades to a row with
    an error message instead of breaking the whole comparison.
    """
    params = {}
    ...
    if method_name == "backdoor.generalized_linear_model":
        # DoWhy's GLM estimator requires an explicit statsmodels family.
        # Gaussian ≈ linear regression but exposes the GLM machinery (planners
        # can later switch links); it's a safe, always-available default.
        try:
...
def _extract_pvalue(sig) -> float:
    """DoWhy returns significance p-values as float, dict, array or tuple
    depending on the estimator. Normalise to a single conservative float
    (the largest value when several are returned), or NaN if unavailable.
    """
    try:
...
_REFUTER_BATTERY = [
    ...
    # Linear effect-strength params so this refuter works for CONTINUOUS
    # treatments too (the default tries to fit a classifier on the treatment
    # and errors out on continuous levers like price).
...
def _refuter_verdict(theta: float, new_effect: float, expectation: str) -> str:
    """Direction-aware robustness verdict.

    expectation='stable' → robust when the refuted effect stays close to the
    original (random common cause, data subset, unobserved confounder).
    expectation='to_zero' → robust when the placebo effect collapses toward 0
    (a non-zero placebo effect signals a spurious estimate).
    """
    if not np.isfinite(new_effect) or abs(theta) < 1e-12:
        return "—"
...
def _run_refutation_battery(m, est, e, theta: float,
    ...
    """Run the selected refuters and return one summary row each."""
...
def _sku_causal_effect(panel, sku, sku_col, date_col, sales_col,
                       lever, lever_pool):
    """Fit a simple linear causal model for one already-forecasted SKU.

    Estimates the effect of `lever` on demand using that SKU's own history
    from the forecast panel, controlling for the other configured levers and
    month-of-year. Returns
    (theta, recent_avg_demand, recent_avg_lever, n_periods, error_str).
    `theta` is the causal effect per +1 unit of the lever.
    """
    try:
        ...
        m = CausalModel(data=dd, treatment=lever, outcome=sales_col,
...
def _reliability_callout(verdict: str):
    """Map a technical robustness verdict to a plain-business confidence level.

    Returns (streamlit_level, headline, plain_explanation) where level is one of
    'success' | 'info' | 'warning' so the result reads like advice, not stats.
    """
    v = str(verdict or '')
    if 'Robust' in v:
...
# Business-friendly labels for the three things a planner can ask.
_T_IMPACT = "📈 How much does a lever move demand?"
_T_WHATIF = "🔮 Try a specific change (what-if)"
_T_DRIVERS = "🏆 Which levers matter most?"


def render_causal_tab():
    st.header("Causal AI — what actually drives your demand")
    st.info(
        "Most charts only show what **moves together** with demand. Causal AI "
        "answers the harder question every planner really asks: **“If I change "
        "this lever — price, promo, discount, scheme days — how much will demand "
        "actually move?”** It automatically accounts for your other business "
        "drivers so you get the *real* effect, not a coincidence.")
    if CausalModel is None or graphviz is None:
        st.error("Install `dowhy` and `graphviz` for this feature: "
                 "`pip install dowhy graphviz`")
        return
    eda = st.session_state.get('eda_object')
    if eda is None:
        st.info("Run **Step 2 · EDA** first (pick a scope and click *Run EDA*) — "
                "Causal AI analyses that prepared series.")
        return
    try:
        ...
        # ── Merge in the Input-configuration exogenous variables ─────────
        # The EDA object collapses to date + sales (see
        # _prepare_dataframe_before_cleaning), so the user's configured
        # exogenous drivers (price, promo, discount, …) would otherwise be
        # invisible to Causal AI. render_unified_eda_tab stashes an aligned,
        # numeric exog frame; merge it on date so every configured exog is
        # available as a treatment AND can be controlled for as a confounder.
        exog_cols: List[str] = []
        _cx = st.session_state.get('causal_exog_df')
        if isinstance(_cx, pd.DataFrame) and not _cx.empty:
            try:
                features_df['date'] = pd.to_datetime(features_df['date'])
                _cx2 = _cx.copy()
                _cx2.index = pd.to_datetime(_cx2.index)
                _new = [c for c in _cx2.columns if c not in features_df.columns]
                features_df = features_df.merge(
                    _cx2[_new], left_on='date', right_index=True, how='left')
                features_df[_new] = features_df[_new].ffill().fillna(0)
                exog_cols = _new
            except Exception as _mex:
                st.caption(f"⚠ Could not merge configured exogenous variables: {_mex}")
        if exog_cols:
            st.success(
                f"✅ Automatically accounting for the **{len(exog_cols)}** business "
                f"driver(s) you set up: {', '.join(f'**{c}**' for c in exog_cols)}.")
        else:
            st.caption(
                "Tip: add your business drivers (price, promo, discount…) in "
                "**Step 2b · Input configuration** and re-run EDA — Causal AI will "
                "then factor them in automatically.")
        ...
        task = st.selectbox("What do you want to find out?",
                            [_T_IMPACT, _T_WHATIF, _T_DRIVERS])

        if task == _T_IMPACT:
            st.markdown("**Step 1 — Pick the lever(s) you want to test.** "
                        "We'll measure how much each one actually moves demand.")
            treatments = st.multiselect(
                "Lever(s) to test (e.g. price, promo, discount)",
                potential, key='causal_treatments',
                help="Choose the business levers you control and want to understand. "
                     "Everything else is treated as a background driver to adjust for.")

            # ---- Sensible defaults so a non-technical user never has to touch
            # the expert knobs below. We auto-account for all configured business
            # drivers and use the standard estimator + full reliability checks.
            confounders_pool = [c for c in potential if c not in treatments]
            if 'causal_confounders' not in st.session_state and exog_cols:
                st.session_state['causal_confounders'] = [
                    c for c in exog_cols if c in confounders_pool]

            with st.expander("⚙️ Advanced settings (optional — defaults work for most cases)"):
                st.caption("Most planners can skip this. We already adjust for your "
                           "configured business drivers and pick a sound default method.")
                confounders = st.multiselect(
                    "Other factors to adjust for",
                    confounders_pool, key='causal_confounders',
                    help="Background drivers held fixed so the lever's effect is isolated.")
                force_exog = True
                if exog_cols:
                    force_exog = st.checkbox(
                        "Always adjust for every configured business driver (recommended)",
                        value=True, key='causal_force_exog')
                effective_confounders = list(confounders)
                if force_exog:
                    for c in exog_cols:
                        if c not in treatments and c not in effective_confounders:
                            effective_confounders.append(c)
                iv_pool = [c for c in potential if c not in treatments and c not in effective_confounders]
                instruments = st.multiselect(
                    "Instruments (advanced — a nudge that moves the lever but not "
                    "demand directly)", iv_pool, key='causal_instruments')
                em_pool = [c for c in potential if c not in treatments]
                effect_modifiers = st.multiselect(
                    "Segments the effect may differ across (advanced)",
                    em_pool, key='causal_effect_modifiers')

                _probe = treatments[0] if treatments else potential[0]
                _cat = _causal_estimator_catalog(features_df, _probe)
                _method_labels = {mn: lbl for mn, lbl, _ in _cat}
                method_choices = [mn for mn, _, ok in _cat if ok]
                if instruments:
                    method_choices.append("iv.instrumental_variable")
                    _method_labels["iv.instrumental_variable"] = "Instrumental variable (IV / 2SLS)"
                chosen_methods = st.multiselect(
                    "Calculation method(s) — the first is the headline number",
                    method_choices, default=["backdoor.linear_regression"],
                    format_func=lambda mn: _method_labels.get(mn, mn),
                    key='causal_methods')
                _ref_labels = {mn: l for mn, l, _, _ in _REFUTER_BATTERY}
                chosen_refuters = st.multiselect(
                    "Reliability cross-checks to run",
                    [mn for mn, _, _, _ in _REFUTER_BATTERY],
                    default=[mn for mn, _, _, _ in _REFUTER_BATTERY],
                    format_func=lambda mn: _ref_labels.get(mn, mn),
                    key='causal_refuters')
                compute_ci = st.checkbox(
                    "Show the plausible range (confidence interval)",
                    value=True, key='causal_ci')
                st.markdown("**How the levers connect (the assumed cause map):**")
                if treatments:
                    st.graphviz_chart(build_causal_graph(
                        treatments, outcome, effective_confounders,
                        instruments, effect_modifiers))

            # Fallback defaults if the expander widgets somehow weren't created
            # (defensive — Streamlit runs expander bodies, so normally set above).
            effective_confounders = locals().get('effective_confounders',
                                                  list(exog_cols))
            chosen_methods = locals().get('chosen_methods', ["backdoor.linear_regression"])
            chosen_refuters = locals().get('chosen_refuters',
                                           [mn for mn, _, _, _ in _REFUTER_BATTERY])
            instruments = locals().get('instruments', [])
            effect_modifiers = locals().get('effect_modifiers', [])
            compute_ci = locals().get('compute_ci', True)
            _method_labels = locals().get('_method_labels', {})

            if treatments:
                st.markdown("**Step 2 — Measure the impact.**")
                if not chosen_methods:
                    st.info("Pick at least one calculation method in Advanced settings.")
                if st.button("📊 Measure impact on demand", key='causal_run',
                             type='primary') and chosen_methods:
                    estimates, refutations, method_rows = [], [], []
                    estimand_texts: Dict[str, str] = {}
                    with st.spinner("Measuring the real impact and running reliability checks…"):
                        for t in treatments:
                            try:
                                model_kwargs = dict(data=features_df, treatment=t,
                                                    outcome=outcome,
                                                    common_causes=effective_confounders)
                                if instruments:
                                ...
                                # Run every chosen estimator; first chosen = headline.
                                headline_e, headline_theta = None, np.nan
                                for mi, method_name in enumerate(chosen_methods):
                                    e, err = _estimate_one_method(m, est, method_name)
                                    if e is None:
                                        method_rows.append({
                                            'Treatment': t, 'Method': _method_labels.get(method_name, method_name),
                                            'Causal Effect': np.nan, 'CI low': np.nan, 'CI high': np.nan,
                                            'p-value': np.nan, 'Note': err})
                                        continue
                                    theta = float(e.value)
                                    ...
                                    method_rows.append({
                                        'Treatment': t, 'Method': _method_labels.get(method_name, method_name),
                                        'Causal Effect': theta, 'CI low': ci_lo, 'CI high': ci_hi,
                                        'p-value': pval, 'Note': ''})
                                    if mi == 0:
                                ...
                                interp, elas = _causal_interpretation(
                                    t, outcome, headline_theta, features_df)
                                rb = _run_refutation_battery(
                                    m, est, headline_e, headline_theta, chosen_refuters)
                                ...
                                estimates.append({
                                    'Treatment': t,                     # canonical key (what-if consumer)
                                    'Causal Estimate': headline_theta,  # canonical value (what-if consumer)
                                    'Treatment (lever)': t,
                                    'Causal Effect (per +1 unit)': headline_theta,
                                    'Elasticity (% per +1%)': elas,
                                    'Robustness': overall,
                                    'Interpretation': interp,
                                })
                                for row in rb:
                                    refutations.append({'Treatment': t, **row})
                                if len(treatments) == 1:
                                    st.session_state['causal_model'] = m
                                    st.session_state['causal_single_estimate'] = headline_e
                            except Exception as ex:
                                estimates.append({
                                    'Treatment': t, 'Causal Estimate': np.nan,
                                    'Treatment (lever)': t,
                                    'Causal Effect (per +1 unit)': np.nan,
                                    'Elasticity (% per +1%)': np.nan,
                                    'Robustness': '—',
                                    'Interpretation': f"Could not estimate: {ex}",
                                })
                    st.session_state['causal_estimate'] = pd.DataFrame(estimates)
                    st.session_state['causal_refutation'] = pd.DataFrame(refutations)
                    st.session_state['causal_method_comparison'] = pd.DataFrame(method_rows)
                    st.session_state['causal_estimands'] = estimand_texts
                    st.session_state['causal_variables'] = {
                        'treatments': treatments, 'outcome': outcome,
                        'confounders': effective_confounders,
                        'instruments': instruments,
                        'effect_modifiers': effect_modifiers}

            # ════════════════ RESULTS — in plain business language ═══════════
            _ce = st.session_state.get('causal_estimate')
            if isinstance(_ce, pd.DataFrame) and not _ce.empty:
                st.markdown("### What we found")
                _mc = st.session_state.get('causal_method_comparison')
                for _, row in _ce.iterrows():
                    lever = row.get('Treatment (lever)', '')
                    theta = row.get('Causal Effect (per +1 unit)')
                    elas = row.get('Elasticity (% per +1%)')
                    interp = row.get('Interpretation', '')
                    st.markdown(f"#### {lever}")
                    if pd.isna(theta):
                        st.warning(interp or "Could not estimate this lever.")
                        st.divider()
                        continue
                    # Plain one-liner + the headline numbers as metrics.
                    direction = "lift" if theta >= 0 else "reduce"
                    mc1, mc2 = st.columns(2)
                    mc1.metric(f"Impact of +1 unit of {lever}",
                               f"{theta:,.1f} units",
                               help=f"A one-unit increase in {lever} is estimated to "
                                    f"{direction} demand by about {abs(theta):,.1f} units.")
                    if pd.notna(elas):
                        mc2.metric("Sensitivity", f"{elas:,.1f}% per 1%",
                                   help="Percent change in demand for every 1% change "
                                        "in this lever.")
                    st.markdown(interp)
                    # Reliability as a coloured callout (advice, not statistics).
                    _lvl, _head, _expl = _reliability_callout(row.get('Robustness', ''))
                    getattr(st, _lvl)(f"**{_head}** — {_expl}")
                    st.divider()

                # ---- Everything technical lives behind one expander ----
                with st.expander("🔬 Technical details (for analysts)"):
                    if isinstance(_mc, pd.DataFrame) and not _mc.empty:
                        st.markdown("**Cross-method comparison** — the same effect, "
                                    "computed several ways. Close agreement = trustworthy.")
                        st.dataframe(_mc.style.format({
                            'Causal Effect': '{:,.3f}', 'CI low': '{:,.3f}',
                            'CI high': '{:,.3f}', 'p-value': '{:,.3g}'}, na_rep='—'),
                            use_container_width=True, hide_index=True)
                    _rf = st.session_state.get('causal_refutation')
                    if isinstance(_rf, pd.DataFrame) and not _rf.empty:
                        st.markdown("**Reliability cross-checks** — we re-test the result "
                                    "by injecting noise, shuffling the lever (placebo), "
                                    "dropping data, and adding a hidden factor. A robust "
                                    "effect barely moves (placebo should fall to ~0).")
                        _rcols = ['Treatment', 'Refuter', 'Refuted effect', 'Verdict', 'p-value']
                        st.dataframe(
                            _rf[[c for c in _rcols if c in _rf.columns]].style.format({
                                'Refuted effect': '{:,.3f}', 'p-value': '{:,.3g}'}, na_rep='—'),
                            use_container_width=True, hide_index=True)
                    if st.session_state.get('causal_estimands'):
                        st.markdown("**Formal estimand & assumptions (DoWhy):**")
                        for t, txt in st.session_state['causal_estimands'].items():
                            st.markdown(f"_Lever: {t}_")
                            st.code(txt)

        elif task == _T_WHATIF:
            st.markdown("Pick a SKU you've already forecasted, choose a lever, set a "
                        "new value, and see the demand we'd expect.")
            cfg_wf = st.session_state.get('app_cfg')
            fc_results = st.session_state.get('forecast_results') or []
            panel_wf = (st.session_state.get('_panel_cache') or {}).get('panel')
            forecasted_skus = [r.sku for r in fc_results]

            if forecasted_skus and cfg_wf is not None and panel_wf is not None:
                sku_col_wf = cfg_wf['sku_col']
                date_col_wf = cfg_wf['date_col']
                sales_col_wf = cfg_wf['sales_col']
                # Levers = the numeric business drivers configured in Input config
                # (plus any planned-event flags), that actually exist in the panel.
                lever_pool = [c for c in (cfg_wf.get('exog_user_numeric') or [])
                              if c in panel_wf.columns
                              and pd.api.types.is_numeric_dtype(panel_wf[c])]
                lever_pool += [c for c in panel_wf.columns
                               if c.startswith('evt_') and c not in lever_pool]
                if not lever_pool:
                    st.info("No numeric business levers available. Add numeric "
                            "exogenous variables (price, promo, discount…) in "
                            "**Step 2b · Input configuration** and re-run the forecast.")
                else:
                    c1, c2 = st.columns(2)
                    sel_sku = c1.selectbox(
                        f"Which forecasted {sku_col_wf}? ({len(forecasted_skus)} available)",
                        forecasted_skus, key='causal_whatif_sku')
                    lever = c2.selectbox("Lever to change", lever_pool,
                                         key='causal_whatif_lever')
                    theta, base_dem, base_lev, nobs, err = _sku_causal_effect(
                        panel_wf, sel_sku, sku_col_wf, date_col_wf,
                        sales_col_wf, lever, lever_pool)
                    if err:
                        st.warning(f"Can't run what-if for **{sel_sku}** / **{lever}**: {err}")
                    else:
                        st.caption(
                            f"Based on **{nobs}** periods of history for {sel_sku} · "
                            f"recent average **{lever}** ≈ **{base_lev:,.2f}** · "
                            f"recent average demand ≈ **{base_dem:,.0f}**")
                        new_val = st.number_input(
                            f"Set **{lever}** to:", value=float(round(base_lev, 2)),
                            key='causal_whatif_val')
                        if st.button("🔮 Show expected demand", type='primary'):
                            delta = (new_val - base_lev) * theta
                            expected = max(0.0, base_dem + delta)
                            d1, d2, d3 = st.columns(3)
                            d1.metric("Recent average demand", f"{base_dem:,.0f}")
                            d2.metric("Expected demand after change", f"{expected:,.0f}")
                            d3.metric("Difference", f"{expected - base_dem:,.0f}",
                                      delta=f"{expected - base_dem:,.0f}")
                            st.markdown(
                                f"For **{sel_sku}**, moving **{lever}** from ~{base_lev:,.2f} "
                                f"to **{new_val:,.2f}** is expected to "
                                f"**{'increase' if delta >= 0 else 'decrease'} demand by "
                                f"about {abs(delta):,.0f} units per period** "
                                f"(effect ≈ {theta:,.2f} units per unit of {lever}, after "
                                f"accounting for your other drivers).")
                            st.caption("Estimated from this SKU's own history via a causal "
                                       "model — not just a correlation.")
            elif st.session_state.get('causal_model'):
                # Fallback: counterfactual on the current EDA single series
                # (when no multi-SKU forecast run is available this session).
                st.caption("No forecast run found — using the current EDA series instead.")
                ...
                _dates = pd.to_datetime(features_df['date'])
                idx = st.slider(
                    f"Which month? (1 = earliest, {len(features_df)} = latest)",
                    0, len(features_df) - 1, len(features_df) - 1, format="%d")
                selected = features_df.iloc[[idx]]
                _when = _dates.iloc[idx].strftime('%b %Y') if idx < len(_dates) else f"row {idx}"
                orig_t = float(selected[treatment].iloc[0])
                st.caption(f"Selected month: **{_when}** · current **{treatment}** = "
                           f"**{orig_t:,.2f}**")
                cf_val = st.number_input(f"Set **{treatment}** to:", value=orig_t)
                if st.button("🔮 Show expected demand", type='primary'):
                    try:
                        _theta = float(e.value)
                        orig_out = float(selected[outcome].iloc[0])
                        _chg = (cf_val - orig_t) * _theta
                        cf_out = max(0.0, orig_out + _chg)
                        c1, c2, c3 = st.columns(3)
                        c1.metric("Demand as it was", f"{orig_out:,.0f}")
                        c2.metric("Expected demand after change", f"{cf_out:,.0f}")
                        c3.metric("Difference", f"{cf_out - orig_out:,.0f}",
                                  delta=f"{cf_out - orig_out:,.0f}")
                        st.markdown(
                            f"Setting **{treatment}** to **{cf_val:,.2f}** in **{_when}** "
                            f"would be expected to **{'increase' if _chg >= 0 else 'decrease'} "
                            f"demand by {abs(_chg):,.0f} units** vs what actually happened.")
                    except Exception as ex:
                        st.error(f"Could not compute the what-if: {ex}")
            else:
                st.info("Run a forecast first (**Step 4 · Forecast**) so you can pick any "
                        "forecasted SKU here — or run an impact analysis on the EDA "
                        "series above.")

        else:  # _T_DRIVERS — rank which levers matter most
            st.markdown("Ranks **every** factor by how much it actually moves demand, "
                        "so you know where to focus first.")
            use_all_conf = st.checkbox(
                "Account for all other drivers when testing each one (recommended)",
                value=True)
            if st.button("🏆 Rank the levers", type='primary'):
                results = []
                pb = st.progress(0, text="Testing each lever…")
                for i, t in enumerate(potential):
                    ...
                    pb.progress((i + 1) / len(potential), text=f"Testing {t}…")
                pb.empty()
                ...
                if not rdf.empty:
                    ...
                    _top = rdf.iloc[0]
                    st.success(f"**Biggest lever: {_top['Lever']}** — it moves demand the "
                               f"most ({_top['Impact on demand']:,.1f} units per 1-unit change). "
                               "Focus your planning here first.")
                    st.caption("Positive = raising the lever raises demand; negative = "
                               "raising it lowers demand. Bars are ranked by size of impact.")
                    fig = px.bar(rdf.head(15), x='Impact on demand', y='Lever',
                                  orientation='h', title='Which levers move demand the most')
                    fig.update_layout(yaxis={'categoryorder': 'total ascending'})
                    st.plotly_chart(fig, use_container_width=True)
                    with st.expander("See the full ranked table"):
                        st.dataframe(rdf.style.format({'Impact on demand': '{:,.4f}'}),
                                     use_container_width=True, hide_index=True)
    except Exception as e:
...
def render_whatif_tab():
    ...
    st.caption(
        "Change a lever (price, promo, scheme days…) and see the impact on the "
        "forecast. Two engines: **causal adjustment** applies the DoWhy effect "
        "you estimated (model-agnostic), while **model re-forecast** feeds the "
        "modified driver back through Prophet / AutoARIMA / SARIMAX.")
    result = st.session_state.get('ss_forecast')
    if result is None:
        st.info("Run a single-series forecast first: **Step 4 · Forecast → "
                "B. Single-SKU multi-model competition**.")
        return
    ...
    if causal_available:
        ...
        if len(treatments) == 1 and treatments[0] == rule['Feature']:
            use_causal = st.checkbox(
                "Apply causal estimate from DoWhy (model-agnostic)", value=True,
                help="Uses the estimated causal effect of this lever to adjust "
                     "the forecast directly. Uncheck to instead re-forecast "
                     "through the underlying model with the modified driver.")
...
def render_unified_eda_tab(cfg):
    ...
    # ---- Exogenous drivers from the Input configuration ----
    # The EDA object collapses to date+sales, so we build a separate, aligned,
    # numeric exog frame here (for the current scope) and stash it for the
    # Causal AI tab. Numeric exog aggregate by mean across SKUs (portfolio),
    # categoricals by mode and are one-hot encoded so DoWhy gets numeric drivers.
    _ex_num = [c for c in (cfg.get('exog_user_numeric') or []) if c in df.columns]
    _ex_cat = [c for c in (cfg.get('exog_user_categorical') or []) if c in df.columns]
    causal_exog_df = None
    if _ex_num or _ex_cat:
        try:
            if scope == "Single SKU (drill-down)":
                _exsrc = df[df[sku_col] == sel_sku][[date_col] + _ex_num + _ex_cat].copy()
            else:
                _agg = {c: 'mean' for c in _ex_num}
                for c in _ex_cat:
                    _agg[c] = lambda s: (s.mode().iloc[0] if not s.mode().empty
                                         else (s.iloc[0] if len(s) else None))
                _exsrc = df.groupby(date_col).agg(_agg).reset_index()
            _exsrc[date_col] = pd.to_datetime(_exsrc[date_col], errors='coerce')
            _exsrc = (_exsrc.dropna(subset=[date_col])
                            .set_index(date_col).sort_index())
            if _ex_cat:
                _exsrc = pd.get_dummies(
                    _exsrc, columns=[c for c in _ex_cat if c in _exsrc.columns],
                    drop_first=True)
            _exsrc = _exsrc.apply(pd.to_numeric, errors='coerce')
            _rs = {'M': 'MS', 'Q': 'QS', 'Y': 'YS'}.get(eda_freq, eda_freq)
            try:
                _exsrc = _exsrc.resample(_rs).mean()
            except Exception:
                pass
            causal_exog_df = _exsrc if not _exsrc.empty else None
        except Exception:
            causal_exog_df = None

    st.info(f"Running EDA on **{scope_label}** at **{eda_freq}** frequency."
            + (f" · {len(causal_exog_df.columns)} exogenous driver(s) attached for Causal AI."
               if causal_exog_df is not None else ""))
    if st.button("Run EDA", use_container_width=True, key='unified_eda_run'):
        try:
            with st.spinner("Building EDA..."):
                ...
                # Attach the configured exogenous drivers for the Causal AI tab.
                st.session_state['causal_exog_df'] = causal_exog_df
                # New scope/series ⇒ stale causal selections; reset so the
                # confounder pre-selection picks up the fresh exog columns.
                for _k in ('causal_confounders', 'causal_treatments',
                           'causal_instruments', 'causal_effect_modifiers'):
                    st.session_state.pop(_k, None)
...
def render_unified_scenarios_tab(cfg):
    ...
    # Prerequisites differ by sub-tab, so guide the user explicitly rather than
    # blocking the whole tab on one missing piece.
    _have_eda = st.session_state.get('eda_object') is not None
    _have_ss_forecast = st.session_state.get('ss_forecast') is not None
    with st.expander("ℹ️ How to use this tab", expanded=not (_have_eda or _have_ss_forecast)):
        st.markdown(
            "- **Causal Effect Estimation** needs a prepared series → run "
            "**Step 2 · EDA** (any scope), then come back.\n"
            "- **What-If Simulation** needs a single-series forecast → run "
            "**Step 2 · EDA → Single SKU**, then **Step 4 · Forecast → "
            "B. Single-SKU multi-model competition**.\n"
            "- Best flow: estimate the causal effect first, then apply it in "
            "What-If to see the impact on the forecast with its uncertainty band.")
        st.caption(
            f"Status — EDA series: {'✓ ready' if _have_eda else '✗ not run'} · "
            f"Single-series forecast: {'✓ ready' if _have_ss_forecast else '✗ not run'}")

    sub = st.radio(
        "Scenario type",
        ["Causal Effect Estimation (DoWhy)", "What-If Feature Simulation"],
        horizontal=True, key='scenario_sub',
        help="Causal: estimate the true (causal) effect of a lever on demand. "
             "What-If: apply a lever change to the forecast and see the impact.",
    )
...
def _render_workspace_hero(cfg):
    """Dashboard landing for the Workspace: a branded banner plus an
    at-a-glance KPI strip computed from the loaded data + config.

    Every metric is wrapped defensively — a missing column or unparseable
    date never breaks the page; that card simply shows a neutral dash.
    """
    df = st.session_state.get('df_raw')

    # Eyebrow / status line driven by how far the workflow has progressed.
    if st.session_state.get('forecasts_run'):
        eyebrow, status_txt = "Workspace · Live", "Forecasts generated"
    elif st.session_state.get('profiled'):
        eyebrow, status_txt = "Workspace · Profiled", "Ready to forecast"
    else:
        eyebrow, status_txt = "Workspace", "Ready to profile"

    tl_section_header(
        title="Forecasting Workspace",
        subtitle=f"{status_txt} — work through the steps below, left to right.",
        eyebrow=eyebrow,
        icon="📊",
    )
    ...
    # 1) Entities (SKUs / chosen forecast grain)
    try:
        n_sku = int(df[cfg['sku_col']].nunique()) if df is not None else None
    except Exception:
        n_sku = None
    cards.append({'icon': '🧬', 'label': 'SKUs',
                  'value': f"{n_sku:,}" if n_sku is not None else "—",
                  'sub': 'unique items', 'accent': '#073e5c', 'accent2': '#0a527a'})
    ...
    try:
        ...
        n_rows = None
    cards.append({'icon': '🗂️', 'label': 'Observations',
                  'value': f"{n_rows:,}" if n_rows is not None else "—",
                  'sub': 'rows of history', 'accent': '#0a9396', 'accent2': '#2a9d8f'})

    # 3) Data span (months/periods covered)
    span_txt = "—"
    try:
        dser = pd.to_datetime(df[cfg['date_col']],
                              format=cfg.get('date_format') or None,
                              errors='coerce').dropna()
        if len(dser):
            months = (dser.max().to_period('M') - dser.min().to_period('M')).n + 1
            span_txt = f"{months}"
    except Exception:
        pass
    cards.append({'icon': '🗓️', 'label': 'History span',
                  'value': span_txt, 'sub': 'periods of data',
                  'accent': '#457b9d', 'accent2': '#073e5c'})

    # 4) Forecast horizon
    cards.append({'icon': '🎯', 'label': 'Horizon',
                  'value': f"{cfg.get('horizon', '—')}",
                  'sub': f"{cfg.get('freq', '')} periods ahead".strip(),
                  'accent': '#ef7602', 'accent2': '#ffa552'})

    # 5) Status pill
    cards.append({'icon': '✅' if st.session_state.get('forecasts_run') else '⏳',
                  'label': 'Status', 'value': status_txt.split(' ')[0],
                  'sub': status_txt, 'accent': '#9b5de5', 'accent2': '#7048c4'})

    tl_kpi_row(cards)


def _render_sidebar_nav():
    """Compact branding + page navigation in the sidebar.

    Returns 'config' or 'workspace'. The sidebar stays slim — just the
    logo and a page switch — because input and all configuration now live
    on the dedicated 'Setup & Configuration' page in the main area.
    """
    with st.sidebar:
        tl_src = (_load_logo_b64('times Lens.png')
                  or _load_logo_b64('Times Lens.png')
                  ...
                  or _load_logo_b64('timeslens.png')
                  or _load_logo_b64('TimesLens.png'))
        _logo = (f'<img src="{tl_src}" alt="Time Lens" '
                 f'style="max-width:150px;max-height:54px;height:auto;" />'
                 if tl_src else
                 '<div style="font-weight:800;font-size:1.05rem;letter-spacing:0.04em;'
                 'color:#073e5c;">TIME <span style="color:#ef7602;">LENS</span></div>')
        st.markdown(
            f"<div style='display:flex;flex-direction:column;align-items:center;"
            f"padding:0.15rem 0 0.6rem 0;border-bottom:1px solid #e5e9ee;"
            f"margin-bottom:0.6rem;'>{_logo}"
            f"<div style='font-size:0.66rem;color:#94a3b8;letter-spacing:0.08em;"
            f"text-transform:uppercase;font-weight:700;margin-top:4px;'>Time Lens</div>"
            f"</div>",
            unsafe_allow_html=True,
        )

        # Default to the config page until a forecast has been configured;
        # afterwards remember whatever the user last selected (radio key).
        configured = st.session_state.get('app_cfg') is not None
        choice = st.radio(
            "Navigation",
            ["⚙️ Setup & Configuration", "📊 Workspace"],
            index=(1 if configured else 0),
            key="_nav_page",
            label_visibility="collapsed",
        )

        # Lightweight status hint so users know whether they can run yet.
        if configured:
            st.caption("✅ Configured — Workspace is ready to run.")
        else:
            st.caption("① Load data & configure on the Setup page.")

    return 'config' if choice.startswith("⚙️") else 'workspace'
...
def main():  # noqa: F811 — extended main supersedes the earlier one
    ...
    # Top-level navigation. Input + all configuration now live on their own
    # dedicated page ("Setup & Configuration") rendered in the main content
    # area; the 8-step workflow lives on the "Workspace" page. The config
    # form writes the resolved cfg into st.session_state['app_cfg'] so the
    # workflow tabs can read it without the widgets being on-screen.
    ...
    # --- Workspace page: the standard forecasting workflow ---
    cfg = st.session_state.get('app_cfg')
    if cfg is None:
        st.info(
            "⚙️ Open **Setup & Configuration** from the sidebar to load your "
            "data and configure the forecast. Once configured, switch back to "
            "the **Workspace** to run the workflow."
        )
        return

    # --- Dashboard hero: banner + at-a-glance KPI strip ---------------
    _render_workspace_hero(cfg)
